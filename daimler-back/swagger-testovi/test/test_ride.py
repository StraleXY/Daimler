...
class RideTest(unittest.TestCase):
    ...
    def test_05_create_ride(self):
        ...
        self.assertEqual(response_body['vehicleType'], 'coupe')
    ...
    def test_29_end_ride(self):
        ...
        response_body = response.json()
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response_body['message'], 'Cannot end a ride that is not in status ACTIVE!')

    def test_30_cancel_ride_unauthorized(self):
        response = send_put_request(data={}, url=f'{self.base_path}/{self.__class__.ride_id}/cancel')
        self.assertEqual(response.status_code, 401)

    def test_31_cancel_ride_forbidden(self):
        response = send_put_request(data={}, url=f'{self.base_path}/{self.__class__.ride_id}/cancel', jwt=self.admin)
        self.assertEqual(response.status_code, 403)

    def test_32_cancel_ride_not_exist(self):
        response = send_put_request(data={}, url=f'{self.base_path}/1234567/cancel', jwt=self.driver)
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.text, 'Ride does not exist!')
    ...
    def test_34_withdraw_ride_unauthorized(self):
        response = send_put_request(data={}, url=f'{self.base_path}/{self.__class__.ride_id}/withdraw')
        self.assertEqual(response.status_code, 401)

    def test_35_withdraw_ride_forbidden(self):
        response = send_put_request(data={}, url=f'{self.base_path}/{self.__class__.ride_id}/withdraw', jwt=self.admin)
        self.assertEqual(response.status_code, 403)

    def test_36_withdraw_ride_not_exist(self):
        response = send_put_request(data={}, url=f'{self.base_path}/1234567/withdraw', jwt=self.passenger)
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.text, 'Ride does not exist!')
    ...
    def test_37_withdraw_ride(self):
        ...
        response = send_put_request(data={}, url=f'{self.base_path}/{response_body["id"]}/withdraw', jwt=self.passenger)
        response_body = response.json()
        self.assertEqual(response.status_code, 400)
    ...
    def test_38_panic_procedure_ride_unauthorized(self):
        response = send_put_request(data={}, url=f'{self.base_path}/{self.__class__.ride_id}/panic')
        self.assertEqual(response.status_code, 401)

    def test_39_panic_procedure_ride_forbidden(self):
        response = send_put_request(data={}, url=f'{self.base_path}/{self.__class__.ride_id}/panic', jwt=self.admin)
        self.assertEqual(response.status_code, 403)

    def test_40_panic_procedure_ride_not_exist(self):
        response = send_put_request(data={}, url=f'{self.base_path}/1234567/panic', jwt=self.passenger)
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.text, 'Ride does not exist!')

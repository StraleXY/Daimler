...
public class CharterBar extends CharterBase {
    ...
    private void init(final Context context, final AttributeSet attrs) {
        ...
        paintBarBackground = typedArray.getBoolean(R.styleable.Charter_c_paintBarBackground, res.getBoolean(R.bool.default_barPaintBackground));
        int barColor = typedArray.getColor(R.styleable.Charter_c_barColor, res.getColor(R.color.default_barColor));
        int barBackgroundColor = typedArray.getColor(R.styleable.Charter_c_barBackgroundColor, res.getColor(R.color.default_barBackgroundColor));
        barMargin = typedArray.getDimension(R.styleable.Charter_c_barMargin, res.getDimension(R.dimen.default_barMargin));
        barMinHeightCorrection = typedArray.getDimension(R.styleable.Charter_c_barMinHeightCorrection, res.getDimension(R.dimen.default_barMinHeightCorrection));
        typedArray.recycle();
    ...
    public void setBarMinHeightCorrection(float barMinHeightCorrection) {
        if (barMinHeightCorrection <= 0f) return;
    ...
    public void draw(Canvas canvas) {
        ...
        if (!anim) valuesTransition = values.clone();

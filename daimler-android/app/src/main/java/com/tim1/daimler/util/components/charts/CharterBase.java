...
class CharterBase extends View {
    ...
    private ICharterAnimListener animListener;
    ...
    private void init(final Context context, final AttributeSet attrs) {
        ...
        anim = typedArray.getBoolean(R.styleable.Charter_c_anim, res.getBoolean(R.bool.default_animState));
        animDuration = checkAnimDuration(typedArray.getInt(R.styleable.Charter_c_animDuration, res.getInteger(R.integer.default_animDuration)));
        showGridLinesX = typedArray.getBoolean(R.styleable.Charter_c_showGridLinesX, res.getBoolean(R.bool.default_showGridLinesX));
        showGridLinesY = typedArray.getBoolean(R.styleable.Charter_c_showGridLinesY, res.getBoolean(R.bool.default_showGridLinesY));
        gridLinesCount = typedArray.getInteger(R.styleable.Charter_c_gridLinesCount, res.getInteger(R.integer.default_gridLinesCount));
        int gridLinesColor = typedArray.getColor(R.styleable.Charter_c_gridLinesColor, res.getColor(R.color.default_gridLinesColor));
        float gridLinesStrokeSize = typedArray.getDimension(R.styleable.Charter_c_gridLinesStrokeSize, res.getDimension(R.dimen.default_gridLinesStrokeSize));

        setWillNotDraw(!typedArray.getBoolean(R.styleable.Charter_c_autoShow, res.getBoolean(R.bool.default_autoShow)));
        typedArray.recycle();
    ...
    private void calculateNextAnimStep(float animationProgress) {
        ...
        for (int i = 0; i < valuesTransition.length; i++) valuesTransition[i] = Math.min(step, values[i]);
        if (animListener != null) animListener.onAnimProgress(animationProgress);
    }
    ...
    public void setAnimListener(ICharterAnimListener animListener) {
    ...
    void playAnimation() {
        ...
        animator.addListener(new Animator.AnimatorListener() {
            ...
            @Override public void onAnimationEnd(Animator animation) {
                animFinished = true;
                if (animListener != null) animListener.onAnimFinish();
            }

            @Override public void onAnimationCancel(Animator animation) {
                if (animListener != null) animListener.onAnimCancel();
            }
